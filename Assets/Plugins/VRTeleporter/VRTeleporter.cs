...
public class VRTeleporter : MonoBehaviour
{
    ...
    public void Teleport()
    {
        if (groundDetected)
        {
            bodyTransform.position = (groundPos + lastNormal * 0.1f) - new Vector3(Camera.main.transform.localPosition.x, 0.0f, Camera.main.transform.localPosition.z);
        }
        else
        {
    ...
    public Vector3 AnimatedTeleport()
    {
        if (groundDetected)
        {
            return (groundPos + lastNormal * 0.1f);
        }
        else
        {
            Debug.Log("Ground wasn't detected");
